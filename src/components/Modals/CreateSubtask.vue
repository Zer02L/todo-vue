<template>
  <div>
    <b-modal
      ...
      :title="
        getSelectedTaskId
          ? 'Создать подзадачу для ' + getTaskById(getSelectedTaskId).name
          : 'Создать подзадачу'
      "
    ...
    >
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-12">
          <v-select
            placeholder="Веберете задачу для этой подзадачи"
            v-model="parentId"
            :options="getTasks"
            :reduce="(task) => task.id"
            label="name"
            :selectable="
              (option) =>
                !(
                  option.subTasks.length >= getDefinedValues.maxSubtaskLength ||
                  option.state == 0
                )
            "
      ...
      <div class="row mb-3">
        <label for="task" class="col-md-12 col-form-label">Подзадача:</label>
        <div class="col-md-12">
          <input
            type="text"
            name="name"
      ...
      >
        <template v-if="$v.$invalid"
          >Пожалуйста, проверьте форму перед подтверждением</template
        >
        <template v-else>Созать подзадачу</template>
      </button>
    </b-modal>
  </div>
</template>
...
<style scoped>
#CreateSubtaskModal label {
  font-weight: bold;
}
#CreateSubtaskModal .btn-disabled {
  opacity: 0.2;
}
</style>
