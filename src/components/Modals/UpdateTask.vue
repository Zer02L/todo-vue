        ...
        <div class="col-md-8">
          ...
          >
            <template v-if="$v.$invalid"
              >Проверьте формы перед изменением</template
            >
  ...
  methods: {
    ...
    save() {
      if (
        ...
        this.setError({
          error: "You did not make any changes to the task information.",
        });
      } else {
        this.updateTask({
          id: this.task.id,
    ...
    selectState(id) {
      let success = null;
      if (id == 0) {
        // before the task is closed, we should check its subtasks.
        let found = _find(this.task.subTasks, { state: 1 });
        if (!found) {
          // if there is at least one open subtask for the task, so we can not close it
          success = true;
        }
      } else {
        success = true;
      ...
      if (success) {
        ...
        this.setError({
          error: "You can not close this task before all subtasks are closed.",
        });
      }
    },
    delTask() {
      let payload = { id: this.task.id };
      this.deleteTask(payload).then(() => {
        this.$bvModal.hide("UpdateTaskModal");
        this.reset();
      });
    },
