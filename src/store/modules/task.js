...
const mutations = {
  ...
  updateTask(state, payload) {
    state.taskList[payload.index].level = this.state.all.levels.find(
      (x) => x.id == payload.levelId
    ).name;
...
const actions = {
  async addTask(context, payload) {
    ...
    if (okay) {
      ...
      context.dispatch(
        "messages/setError",
        { error: `You already have a task named "${payload.name}".` },
        { root: true }
      );
    }
  },
  async updateTask(context, payload) {
    try {
      ...
      if (index == -1) {
        context.dispatch(
          "messages/setError",
          { error: "Task could not find! Something is totally wrong!" },
          { root: true }
        );
      } else {
        payload.index = index;
  ...
  async deleteTask(context, payload) {
    ...
    try {
      ...
      if (index == -1) {
        ...
        context.dispatch(
          "messages/setIsSuccess",
          { message: "Your subtask is successfully deleted!" },
          { root: true }
        );
        context.commit("all/updateStorage", null, { root: true });
      }
